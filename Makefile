...
help:
	...
	@echo "$(BLUE)════════════════════════════════════════$(NC)"
	@echo ""
	@echo "$(GREEN)📌 사용자용 (일반 개발자)$(NC)"
	@echo ""
	@echo "$(YELLOW)환경 시작:$(NC)"
	@echo "  make dev       - 개발 환경 시작 ⭐ (이것만 사용하세요)"
	@echo "  make test      - 테스트 환경 시작"
	@echo "  make staging   - 스테이징 환경 시작"
	...
	@echo ""
	@echo "$(GREEN)🔧 개발자용 (컨테이너 내부에서 실행)$(NC)"
	@echo ""
	@echo "$(YELLOW)컨테이너 내부에서만 실행:$(NC)"
	@echo "  docker exec -it soulflow-dev-orchestrator bash"
	@echo "  npm run build     # 타입스크립트 빌드"
	@echo "  npm test          # 유닛 테스트"
	@echo "  npm run lint      # 코드 검사"
	@echo ""
	@echo "$(YELLOW)옵션:$(NC)"
	@echo "  make dev WORKSPACE=/custom/path  - 커스텀 워크스페이스"
	@echo ""
