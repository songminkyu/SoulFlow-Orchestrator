    ...
    <>
      ...
      {action === "generate_client" && (
        <BuilderField label={t("workflows.field_language")}>
          <select className="input input--sm" value={String(node.lang || "typescript")} onChange={(e) => update({ lang: e.target.value })}>
...
export const openapi_descriptor: FrontendNodeDescriptor = {
  ...
  input_schema: [
    ...
    { name: "input", type: "string", description: "node.openapi.input.input" },
  ],
  create_default: () => ({ action: "parse", input: "", method: "", path: "", lang: "typescript" }),
