        ...
        <BuilderRowPair>
          <BuilderField label={t("workflows.action")} required>
            <select autoFocus className="input input--sm" required value={action} onChange={(e) => update({ action: e.target.value })} aria-required="true">
              <optgroup label={t("workflows.changelog_group_changelog")}>{CHANGELOG_ACTIONS.map((a) => <option key={a} value={a}>{t(`node.action.${a}`)}</option>)}</optgroup>
              <optgroup label={t("workflows.changelog_group_license")}>{LICENSE_ACTIONS.map((a) => <option key={a} value={a}>{t(`node.action.${a}`)}</option>)}</optgroup>
            </select>
          </BuilderField>
