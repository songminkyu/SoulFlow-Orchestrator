...
function CookieEditPanel({ node, update, t }: EditPanelProps) {
  ...
  return (
    <>
      ...
      {(action === "parse" || action === "parse_set_cookie" || action === "is_expired") && (
        ...
        <>
          <BuilderRowPair>
            <BuilderField label={t("workflows.field_name")} required>
              <input className="input input--sm" required value={String(node.name || "")} onChange={(e) => update({ name: e.target.value })} placeholder="session" aria-required="true" />
            </BuilderField>
            <BuilderField label={t("workflows.field_value")} required>
              <input className="input input--sm" required value={String(node.value || "")} onChange={(e) => update({ value: e.target.value })} placeholder="abc123" aria-required="true" />
            </BuilderField>
          </BuilderRowPair>
        </>
      )}
...
export const cookie_descriptor: FrontendNodeDescriptor = {
  ...
  create_default: () => ({ action: "parse", input: "", name: "", value: "" }),
