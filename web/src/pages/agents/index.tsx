...
export default function AgentsPage() {
  ...
  async function handle_delete() {
    ...
    try {
      await api.delete(`/api/agent-definitions/${deleteTarget.id}`);
  ...
  return (
    <div className="page">
      <SectionHeader title={t("agents.gallery_title")}>
        <SearchInput
          value={search}
          onChange={setSearch}
      ...
      {isLoading ? (
        ...
        <EmptyState message={t("agents.no_definitions")} />
      ) : (
        <>
          {/* 빌트인 에이전트 */}
          {builtins.length > 0 && (
          ...
          {filtered.length === 0 && search && (
            <EmptyState message={t("agents.no_search_results", { query: search })} />
          )}
        </>
      )}
