...
// ══════════════════════════════════════════
// webhook polling 내부 로직 (from cov3) — fake timers 필요
// ══════════════════════════════════════════

describe("webhook polling — data 있음 → execute 호출 (fake timers)", () => {
  ...
  it("setInterval 발화 → webhook_store.get data 반환 → execute 호출됨", async () => {
    ...
    expect(vi.mocked(execute)).toHaveBeenCalledWith(
      "webhook-flow", "slack", "chat-001", { webhook: webhook_data },
    );
  });
  ...
  it("setInterval 발화 → webhook_store.get null → continue (execute 미호출)", async () => {
