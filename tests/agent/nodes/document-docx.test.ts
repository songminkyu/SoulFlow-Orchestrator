...
import { document_docx_handler } from "../../../src/agent/nodes/document-docx.js";
import type { OrcheNodeDefinition } from "../../../src/agent/nodes/workflow-node.types.js";
import type { OrcheNodeExecutorContext } from "../../../src/agent/nodes/orche-node-executor.js";

describe("document_docx_handler", () => {
  ...
  it("metadata: node_type should be document_docx", () => {
    expect(document_docx_handler.node_type).toBe("document_docx");
  });

  it("execute: should generate DOCX document", async () => {
    const node = createMockNode();
    const ctx = createMockContext();
    const result = await document_docx_handler.execute(node, ctx);
    expect(result.output).toBeDefined();
    expect(typeof result.output.success).toBe("boolean");
  });

  it("execute: should resolve templates in content", async () => {
    const node = createMockNode({ content: "${doc_content}" });
    const ctx = createMockContext({ doc_content: "Template resolved content" });
    const result = await document_docx_handler.execute(node, ctx);
    expect(result.output).toBeDefined();
  });
  ...
  it("test: preview should show action and output path", () => {
    const node = createMockNode();
    const result = document_docx_handler.test(node);
    expect(result.preview).toBeDefined();
    expect(result.preview.action).toBe("create_docx");
  });
