...
// ── from text-splitter-extended.test.ts ──

function make_ts_node(overrides: Record<string, unknown>): OrcheNodeDefinition {
  return {
    node_id: "n1",
    node_type: "text_splitter",
    input_field: "text",
...
function make_ts_ctx(memory: Record<string, unknown> = {}): OrcheNodeExecutorContext {
...
describe("text_splitter_handler — 구분자 기반 분할 (separator path)", () => {
  it("separator + chunk 오버플로 → 새 청크 시작 + overlap 포함", async () => {
    const node = make_ts_node({ input_field: "content", chunk_size: 15, chunk_overlap: 3, separator: "\n\n" });
    const ctx = make_ts_ctx({ content: "Hello World\n\nGoodbye World\n\nAnother" });
  ...
  it("구분자가 있지만 전체 텍스트가 chunk_size 이하 → 1 청크", async () => {
    const node = make_ts_node({ input_field: "t", chunk_size: 1000, chunk_overlap: 0, separator: "\n" });
    const ctx = make_ts_ctx({ t: "line1\nline2\nline3" });
    const r = await text_splitter_handler.execute(node, ctx);
    expect(r.output.chunk_count).toBe(1);
  });

  it("구분자 없는 고정 분할 — overlap 적용", async () => {
    const node = make_ts_node({ input_field: "t", chunk_size: 5, chunk_overlap: 2, separator: undefined });
    const ctx = make_ts_ctx({ t: "0123456789" });
    const r = await text_splitter_handler.execute(node, ctx);
    expect(r.output.chunk_count).toBeGreaterThan(1);
  ...
  it("빈 문자열 → chunk_count=0", async () => {
    const node = make_ts_node({ input_field: "t", chunk_size: 100, chunk_overlap: 0 });
    const ctx = make_ts_ctx({ t: "" });
    const r = await text_splitter_handler.execute(node, ctx);
...
describe("text_splitter_handler — 중첩 필드 접근", () => {
  it("중첩 필드 (a.b) 성공", async () => {
    const node = make_ts_node({ input_field: "doc.body", chunk_size: 100, chunk_overlap: 0 });
    const ctx = make_ts_ctx({ doc: { body: "Some text content here" } });
    const r = await text_splitter_handler.execute(node, ctx);
    expect(r.output.chunk_count).toBeGreaterThan(0);
  });

  it("중첩 필드 없음 → chunk_count=0", async () => {
    const node = make_ts_node({ input_field: "doc.missing_field", chunk_size: 100, chunk_overlap: 0 });
    const ctx = make_ts_ctx({ doc: { other: "value" } });
    const r = await text_splitter_handler.execute(node, ctx);
    expect(r.output.chunk_count).toBe(0);
  ...
  it("필드가 문자열이 아님 → chunk_count=0", async () => {
    const node = make_ts_node({ input_field: "count", chunk_size: 100, chunk_overlap: 0 });
    const ctx = make_ts_ctx({ count: 42 });
    const r = await text_splitter_handler.execute(node, ctx);
    expect(r.output.chunk_count).toBe(0);
  });

  it("memory에 필드 없음 → chunk_count=0", async () => {
    const node = make_ts_node({ input_field: "nonexistent", chunk_size: 100, chunk_overlap: 0 });
    const ctx = make_ts_ctx({});
    const r = await text_splitter_handler.execute(node, ctx);
    expect(r.output.chunk_count).toBe(0);
  });
});
...
describe("text_splitter_handler — test() 경고", () => {
  it("input_field 없음 → warnings 포함", () => {
    const node = make_ts_node({ input_field: "", chunk_size: 1000, chunk_overlap: 200 });
    const r = text_splitter_handler.test(node);
    expect(r.warnings.some(w => w.includes("input_field"))).toBe(true);
  });

  it("chunk_size < 100 → 경고", () => {
    const node = make_ts_node({ input_field: "t", chunk_size: 50, chunk_overlap: 10 });
    const r = text_splitter_handler.test(node);
    expect(r.warnings.some(w => w.includes("chunk_size"))).toBe(true);
  });

  it("chunk_overlap >= chunk_size → 경고", () => {
    const node = make_ts_node({ input_field: "t", chunk_size: 100, chunk_overlap: 100 });
    const r = text_splitter_handler.test(node);
    expect(r.warnings.some(w => w.includes("chunk_overlap"))).toBe(true);
  });

  it("정상 설정 → warnings 없음", () => {
    const node = make_ts_node({ input_field: "t", chunk_size: 500, chunk_overlap: 50 });
    const r = text_splitter_handler.test(node);
    expect(r.warnings).toHaveLength(0);
  });

  it("preview에 separator '(none)' 표시 (separator 없음)", () => {
    const node = make_ts_node({ input_field: "t", chunk_size: 500, chunk_overlap: 50, separator: undefined });
    const r = text_splitter_handler.test(node);
    expect(r.preview.separator).toBe("(none)");
  });

  it("preview에 separator 값 표시", () => {
    const node = make_ts_node({ input_field: "t", chunk_size: 500, chunk_overlap: 50, separator: "\n\n" });
    const r = text_splitter_handler.test(node);
...
describe("text_splitter_handler — non-object 경로 (L65)", () => {
  it("중첩 필드 탐색 중 중간 값이 non-object → else { text=undefined; break } (L65)", async () => {
    const node = make_ts_node({ input_field: "a.b.c" });
    const ctx = make_ts_ctx({ a: "string_not_object" });
    const r = await text_splitter_handler.execute(node, ctx);
