...
describe("package_manager_handler", () => {
  ...
  it("metadata: node_type should be package_manager", () => {
    expect(package_manager_handler.node_type).toBe("package_manager");
  });

  it("execute: should execute handler", async () => {
    const node = createMockNode();
    const ctx = createMockContext();
    const result = await package_manager_handler.execute(node, ctx);
    expect(result.output).toBeDefined();
  });

  it("test: preview should be defined", () => {
    const node = createMockNode();
    const result = package_manager_handler.test(node);
