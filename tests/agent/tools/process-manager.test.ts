 ...
 * - L46: run_command_argv 예외 → catch → "Error: ..." 반환
 */
...
vi.mock("@src/agent/tools/shell-runtime.js", () => ({
  ...
  run_command_argv: vi.fn().mockRejectedValue(new Error("shell execution failed")),
}));

import { ProcessManagerTool } from "@src/agent/tools/process-manager.js";

// ── L46: run_command_argv 예외 → catch ──────────────────────────────────────

describe("ProcessManagerTool — L46: shell 예외 → catch", () => {
  it("list op → run_command_argv 예외 → L46 catch → Error 반환", async () => {
