...
let ext_cleanup_dirs: string[] = [];

afterEach(async () => {
  for (const d of ext_cleanup_dirs) {
...
async function make_ext_workspace(): Promise<string> {
  const d = await mkdtemp(join(tmpdir(), "lock-ext-"));
  ext_cleanup_dirs.push(d);
  return d;
}
