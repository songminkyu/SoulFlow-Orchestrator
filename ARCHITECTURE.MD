...
| Ops/Dashboard | `src/ops/`, `src/dashboard/` | 운영 서비스, 웹 대시보드 |
...
`CommandRouter` (`src/channels/commands/router.ts`)가 10개 슬래시 커맨드를 라우팅:
`/help`, `/stop`, `/render`, `/secret`, `/memory`, `/decision`, `/cron`, `/promise`, `/reload`, `/status`
...
`src/providers/service.ts` — 4개 LLM 프로바이더 관리:
...
`src/providers/circuit-breaker.ts` — 프로바이더별 장애 차단:
...
- 입력: `templates/*.md`, `agents/*.md`, `memory.db`, skills, decisions
...
- SSE 기반 실시간 업데이트
- 포트 충돌 시 자동 fallback (`DASHBOARD_PORT_FALLBACK=1`)

### Ops (`src/ops/service.ts`)

- health tick, watchdog, decision dedupe

## 16. Workflow Events + Task 동기화
...
- 프로바이더 장애: CircuitBreaker 차단 → HealthScorer fallback
...
| `ExecutorProvider` | `"chatgpt" \| "claude_code" \| "phi4_local" \| "openrouter"` | AI 모델 백엔드 |
| `ExecutionMode` | `"once" \| "task" \| "agent"` | 실행 전략 |
| `CircuitState` | `"closed" \| "open" \| "half_open"` | 프로바이더 장애 허용 |
