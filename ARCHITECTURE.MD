...
| Agent Backends | `src/agent/backends/` | 9개 백엔드 통합 (CLI 래퍼 + 네이티브 SDK + OpenAI 호환 + Ollama) |
...
4. 핵심 서비스 생성:
   ...
   - 에이전트 백엔드 조립 (`provider-factory.ts` 팩토리 기반):
     ...
     - `OpenAICompatibleAgent("ollama")` (Ollama 로컬 LLM)
...
`CommandRouter` (`src/channels/commands/router.ts`)가 21개 슬래시 커맨드를 라우팅:
`/help`, `/stop`, `/render`, `/secret`, `/memory`, `/decision`, `/promise`, `/cron`, `/reload`, `/task`, `/status`, `/skill`, `/doctor`, `/agent`, `/stats`, `/verify`, `/guard`, `/workflow`, `/model`, `/mcp`, `/tone`
...
`AgentBackendRegistry` (`src/agent/agent-registry.ts`)가 9개 백엔드를 통합 관리합니다:
...
| `openrouter` | `OpenAICompatibleAgent` | OpenRouter HTTP API (OpenAI 호환) |
| `ollama` | `OpenAICompatibleAgent` | Ollama 로컬 LLM (`http://ollama:11434/v1` 기본값) |
...
| 역할 | 식별자 | 설명 |
|------|--------|------|
| concierge | `role:concierge` | 사용자 직접 대면, 비개발 작업 처리, 개발 감지 시 PM/PL에 위임 |
...
| `AgentBackendId` | `"claude_cli" \| "codex_cli" \| "claude_sdk" \| "codex_appserver" \| "gemini_cli" \| "openai_compatible" \| "openrouter" \| "ollama" \| "container_cli"` | 에이전트 백엔드 |
| `AgentBackend` | 인터페이스 | `run(options)` → `AgentRunResult` |
