...
Query Text
    ...
    |     랭킹된 결과 (우선)
    |
    +---> sqlite-vec KNN (시멘틱 매칭, lazy embed)
              |
              v
          보충 결과
              |
    +---------+
    v
  병합된 결과 셋 (FTS5 우선, 벡터가 남은 슬롯 채움)
```

### 임베딩 전략: Lazy + Content-Hash
...
| 파라미터 | 값 | 근거 |
|---------|-----|------|
| 모델 | `text-embedding-3-small` | OpenRouter 경유, 저비용 |
| 차원 | 256 | 도구 165개, 메모리 수백 건 → 256차원 충분 |
| 최대 문자 | 1500 (도구), 2000 (메모리) | 임베딩 입력 truncate |
| 거리 | L2 (정규화된 벡터) | `cosine_sim = 1 - (L2^2 / 2)` |

### 적용 대상

#### 1. 도구 인덱스 (`src/orchestration/tool-index.ts`)
...
#### 2. 메모리 스토어 (`src/agent/memory.service.ts`)
