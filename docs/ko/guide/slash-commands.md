...
| 커맨드 | 설명 |
|--------|------|
| `/workflow list` | 활성 워크플로우 목록 |
| `/workflow create` | 새 워크플로우 생성 |
| `/workflow cancel <id>` | 실행 중인 워크플로우 취소 |

## 모델

| 커맨드 | 설명 |
|--------|------|
| `/model list` | 설치된 오케스트레이터 LLM 모델 목록 |
| `/model set <name>` | 활성 분류기 모델 변경 |

## MCP

| 커맨드 | 설명 |
|--------|------|
| `/mcp list` | 연결된 MCP 서버 목록 및 상태 |
| `/mcp reconnect <name>` | 특정 MCP 서버 재연결 |

## 톤

| 커맨드 | 설명 |
|--------|------|
| `/tone <스타일>` | 페르소나 톤 설정 (예: `formal`, `casual`, `반말`, `따뜻`, `짧게`, `자세`) |
| `/tone status` | 현재 톤 설정 확인 |
| `/tone reset` | 기본 톤으로 초기화 |
