...
사이드바는 4개 그룹으로 구성됩니다: **Main** (Overview, Chat), **Build** (Prompting, Workflows, Kanban, WBS, Workspace), **Connect** (Channels, Providers), **System** (Secrets, Settings). 슈퍼어드민 사용자에게는 **Admin** 링크가 추가로 표시됩니다. 하단 버튼으로 다크/라이트 테마를 전환할 수 있습니다.
...
| 그룹 | 페이지 | 경로 | 기능 |
|------|--------|------|------|
| Main | Overview | `/` | 런타임 상태 요약, 시스템 메트릭, SSE 실시간 피드 |
| Main | Chat | `/chat` | 웹 기반 에이전트 대화 (마크다운 렌더링 + 코드 하이라이팅) |
| Build | **Prompting** | `/prompting` | 프롬프팅 스튜디오 — 텍스트/이미지/동영상 생성, 에이전트 설계·테스트, 갤러리, A/B 비교 |
| Build | Workflows | `/workflows` | Phase Loop 워크플로우 관리 · 141종 노드 그래프 에디터 · 에이전트 채팅 |
| Build | Kanban | `/kanban` | 드래그앤드롭 칸반 보드 · 자동화 규칙 |
| Build | WBS | `/wbs` | 칸반 카드 계층 트리 뷰 (parent_id 기반) |
| Build | Workspace | `/workspace` | 메모리·세션·스킬·크론·도구·에이전트·템플릿·OAuth·모델·레퍼런스 관리 (10탭) |
| Connect | Channels | `/channels` | 채널 연결 상태 · 글로벌 설정 |
| Connect | Providers | `/providers` | 에이전트 프로바이더 CRUD · Circuit Breaker 상태 |
| System | Secrets | `/secrets` | AES-256-GCM 시크릿 관리 |
| System | Settings | `/settings` | 글로벌 런타임 설정 (섹션 탭, 인라인 편집, ToggleSwitch) |
| Admin | Admin | `/admin` | 팀/사용자 관리 · 전역 프로바이더 · 모니터링 (슈퍼어드민 전용) |
...
`/prompting` — 프롬프트 실험, 에이전트 설계, 멀티모달 생성을 위한 통합 환경. 6개 탭으로 구성됩니다:

| 탭 | 아이콘 | 용도 |
|----|--------|------|
| **Text** | `T` | `{{variable}}` 치환 지원 템플릿 기반 프롬프트 실행 · 모델/temperature/max_tokens 조절 |
| **Image** | `◎` | 이미지 생성 모델 실행 · 프로바이더/모델 선택 |
| **Video** | `▷` | 동영상 생성 모델 실행 · 프로바이더/모델 선택 |
| **Agent** | `🤖` | 전체 에이전트 정의 편집기 + 실시간 테스트 채팅 |
| **Gallery** | `◈` | 에이전트 정의 CRUD 갤러리 · fork/복제 · Agent 탭 연동 |
| **Compare** | `⚖` | 동일 프롬프트를 여러 모델에 병렬 실행 · 결과/비용/레이턴시 나란히 비교 (A/B 테스트) |

### Agent 탭

서브 에이전트를 설계하고 즉시 테스트하는 핵심 도구입니다. 에이전트 정의 폼과 채팅 패널이 한 화면에 통합되어 있습니다:

**정의 필드:**
- `name`, `description`, `icon` — 에이전트 식별 정보
- `role_skill` — 8개 내장 역할 스킬 중 선택 (`role:concierge`, `role:pm`, `role:pl`, `role:implementer`, `role:reviewer`, `role:debugger`, `role:validator`, `role:generalist`) 또는 커스텀
- `soul` / `heart` — 성격과 행동 가이드라인
- `tools` — 쉼표 구분 도구 목록
- `shared_protocols` — 공유 행동 프로토콜 (`clarification-protocol`, `phase-gates`, `error-escalation`, `session-metrics`, `difficulty-guide`)
- `skills` — 추가 스킬 참조
- `use_when` / `not_use_for` — 라우팅 가이드
- `extra_instructions` — 추가 프롬프트 텍스트

**자연어로 생성**: 원하는 에이전트를 자연어로 설명 → AI가 모든 정의 필드 자동 작성.

**테스트 채팅**: 저장 없이 현재 정의로 에이전트를 즉시 실행 — 결과 인라인 표시.

**Gallery 연동**: Gallery에 저장하거나, Gallery에서 에이전트를 불러와 Agent 탭에서 편집.

### Compare 탭

A/B 테스트를 위한 병렬 모델 평가:

1. 모델 타겟 2개 이상 추가 (슬롯마다 프로바이더 + 모델 선택)
2. 시스템 프롬프트, 사용자 프롬프트, temperature, max_tokens 설정
3. 실행 → 결과를 나란히 표시: 생성 텍스트 · 레이턴시(ms) · 예상 비용 · 토큰 수
