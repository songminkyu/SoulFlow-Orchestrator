...
1. **병렬 전문가 협업** + 품질 게이트
2. **시각적 워크플로우 자동화** — 6개 카테고리 42종 노드 타입
3. **샌드박스 실행** — 격리 컨테이너
...
그래프 에디터는 6개 카테고리(flow, data, AI, integration, interaction, advanced)의 42종 노드 타입을 지원합니다. 매 단계마다 LLM 추론이 필요 없는 결정적 자동화 파이프라인에 사용하세요.
