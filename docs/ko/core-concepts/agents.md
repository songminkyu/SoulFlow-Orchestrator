...
가드레일은 결과, 예산, 실패 패턴을 추적하여 무한 루프나 낭비적 에이전트 실행을 방지합니다.

### 예산 계약 (Budget Contract)

모든 태스크 실행에는 에이전트에게 리소스 한도를 미리 전달하는 **예산 계약**이 부여됩니다:

| 한도 | 설명 |
|------|------|
| `max_turns` | 에이전트가 사용할 수 있는 최대 추론 턴 수 |
| `max_tokens` | 실행 컨텍스트의 토큰 예산 |
| `max_retries` | 도구 호출 실패 시 재시도 상한 |

에이전트는 시스템 컨텍스트에서 남은 예산을 확인하여, 작업 범위와 응답 길이를 스스로 조절합니다.

### 결과 인식 세션 재사용 (Outcome-Aware Session Reuse)

세션 종료(성공 또는 실패) 시 결과가 기록됩니다. 재시도 또는 재실행 시:

- **성공한 세션** — 이미 완료된 단계를 건너뛰기 위해 컨텍스트 재사용 가능
- **실패한 세션** — 오염된 상태 상속을 방지하기 위해 컨텍스트 재사용 제외; 에이전트가 새로 시작

### 노벨티 정책 (Novelty Policy)

이전에 시도한 접근 방식을 재시도하기 전에, 에이전트는 결과를 바꿀 수 있는 새로운 정보가 있는지 평가합니다. 새로운 입력 없이 실패한 접근 방식은 자동으로 건너뜁니다.

### 실패 시도 단락 (Failed-Attempt Short-Circuit)

동일 작업이 연속으로 `N`번 실패하면(설정 가능), 무한 재시도 대신 구조화된 실패 보고서와 함께 실행이 단락됩니다. 임계값은 작업 유형별:

```
tool_call 실패       → 3회 시도
LLM 추론 오류        → 2회 시도
네트워크/API 오류     → 지수 백오프, 최대 5회
```

실패 단락은 대시보드에 `guardrail_abort` 이벤트로 진단 정보와 함께 표시됩니다.
