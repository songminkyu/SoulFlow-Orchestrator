...
export class PromiseTool extends Tool {
  readonly name = "promise";
  readonly description = "약속(제약 조건) 조회·설정. action=list|set|get_effective";
  readonly parameters: JsonSchema = {
    type: "object",
    required: ["action"],
    properties: {
      action: { type: "string", enum: ["list", "set", "get_effective"] },
      key: { type: "string", description: "약속 키 (set/list 필터)" },
      value: { type: "string", description: "약속 내용 (set 시 필수)" },
      scope: { type: "string", enum: ["global", "team", "agent"], description: "스코프 (기본 global)" },
      rationale: { type: "string", description: "약속 이유 (선택)" },
  ...
  protected async run(params: Record<string, unknown>): Promise<string> {
    ...
    if (action === "set") {
      ...
      const result = await this.promises.append_promise({
        scope,
        key,
        value,
      ...
      return `약속 ${result.action}: [${scope}] ${key} = ${result.record.value}`;
    }

    if (action === "get_effective") {
      const rows = await this.promises.get_effective_promises();
      if (rows.length === 0) return "(유효 약속 없음)";
