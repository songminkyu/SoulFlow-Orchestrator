/** Decision (결정사항 관리) 노드 핸들러. */

import type { NodeHandler, RunnerContext } from "../node-registry.js";
import type { DecisionNodeDefinition, OrcheNodeDefinition } from "../workflow-node.types.js";
import type { OrcheNodeExecutorContext, OrcheNodeExecuteResult, OrcheNodeTestResult } from "../orche-node-executor.js";
import { resolve_templates } from "../orche-node-executor.js";
import { error_message } from "../../utils/common.js";

export const decision_handler: NodeHandler = {
  node_type: "decision",
  icon: "⚖",
  color: "#795548",
  shape: "rect",
  ...
  async execute(node: OrcheNodeDefinition, ctx: OrcheNodeExecutorContext): Promise<OrcheNodeExecuteResult> {
    const n = node as DecisionNodeDefinition;
  ...
  async runner_execute(node: OrcheNodeDefinition, ctx: OrcheNodeExecutorContext, runner: RunnerContext): Promise<OrcheNodeExecuteResult> {
    const svc = runner.services?.decision;
    ...
    try {
      ...
      runner.logger.warn("decision_node_error", { node_id: n.node_id, error: error_message(err) });
