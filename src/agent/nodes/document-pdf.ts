/** Document Pdf node handler. */

import type { NodeHandler } from "../node-registry.js";
import type { DocumentPdfNodeDefinition, OrcheNodeDefinition } from "../workflow-node.types.js";
import type { OrcheNodeExecuteResult, OrcheNodeTestResult, OrcheNodeExecutorContext } from "../orche-node-executor.js";
import { resolve_templates } from "../orche-node-executor.js";

export const document_pdf_handler: NodeHandler = {
  ...
  async execute(node: OrcheNodeDefinition, ctx: OrcheNodeExecutorContext): Promise<OrcheNodeExecuteResult> {
    const n = node as DocumentPdfNodeDefinition;
    try {
  ...
  test(node: OrcheNodeDefinition): OrcheNodeTestResult {
    ...
    return { preview: { action: "create_pdf", output: n.output }, warnings };
  },
};
