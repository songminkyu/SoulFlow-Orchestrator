...
export function create_channel_wiring(deps: ChannelWiringDeps): ChannelWiringResult {
  ...
  const channel_manager = new ChannelManager({
    ...
    bot_identity,
    on_agent_event: (event, team_id) => broadcaster.broadcast_agent_event(event, team_id),
    on_web_stream: (chat_id, content, done, team_id) => broadcaster.broadcast_web_stream(chat_id, content, done, team_id),
    on_web_rich_event: (chat_id, event) => broadcaster.broadcast_web_rich_event(chat_id, event),
    confirmation_guard,
