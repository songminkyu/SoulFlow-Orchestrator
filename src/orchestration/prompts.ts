...
export const EXECUTION_MODE_DEFINITIONS = [
  ...
  "- guard: on | off — Toggle execution confirmation guard",
  "",
  "### Routing rules:",
  ...
  '"가드 켜줘" → {"mode":"builtin","command":"guard","args":"on"}',
  '"확인 모드 꺼" → {"mode":"builtin","command":"guard","args":"off"}',
  "",
  "## identity",
  "The user asks who the assistant is, what its name is, or what role/identity it has.",
  "No tool call, no executor spawn. The orchestrator answers directly.",
  "Use identity for:",
  "- 너 누구야?, 이름이 뭐야?, 당신은 누구입니까, 정체가 뭐야",
  "- Who are you?, What is your name?, What are you?",
  "IMPORTANT: identity takes priority even if active tasks exist.",
  "",
  "## identity examples",
  '"너 누구야?" → {"mode":"identity"}',
  '"이름이 뭐야?" → {"mode":"identity"}',
  '"당신 정체가 뭔가요?" → {"mode":"identity"}',
  '"who are you?" → {"mode":"identity"}',
  '"what is your name?" → {"mode":"identity"}',
  "",
  "## once",
  ...
  "## once examples",
  'User: "안녕" → {"mode":"once"}',
  'User: "오늘 날씨 알려줘" → {"mode":"once","tools":["web"]}',
  'User: "이 파일 여기에 첨부해줘" → {"mode":"once","tools":["file_transfer"]}',
  'User: "이제 첨부 도구가 사용 가능할거야" → {"mode":"once"}',
  'User: "크론 등록해줘 매일 9시" → {"mode":"once","tools":["scheduling"]}',
  'User: "이전에 만든 PDF 보내줘" → {"mode":"once","tools":["file_transfer"]}',
  'User: "고마워" → {"mode":"once"}',
  'User: "메모리에 이거 저장해줘" → {"mode":"once","tools":["memory"]}',
  'User: "시크릿 등록해줘 API_KEY abc123" → {"mode":"once","tools":["secret"]}',
  'User: "웹 검색해줘 TypeScript 5.0" → {"mode":"once","tools":["web"]}',
  "",
  "## agent examples",
  'User: "아이유에 대해 조사하고 리포트를 PDF로 만들어서 첨부해줘" → {"mode":"agent","tools":["web","filesystem","file_transfer"]}',
  'User: "경쟁사 3곳을 분석하고 비교표를 만들어줘" → {"mode":"agent","tools":["web","filesystem"]}',
  'User: "코드를 분석하고 리팩토링 계획을 세워줘" → {"mode":"agent","tools":["filesystem","shell"]}',
  'User: "최신 뉴스를 수집해서 요약 보고서를 작성해줘" → {"mode":"agent","tools":["web","filesystem"]}',
  'User: "자세한 정보를 찾아서 분석하고 리포트를 만들어줘" → {"mode":"agent","tools":["web","filesystem"]}',
  'User: "이 파일 읽고 요약해줘" → {"mode":"agent","tools":["filesystem"]}',
  'User: "검색하고 비교 분석해줘" → {"mode":"agent","tools":["web"]}',
  "",
  "## task examples",
  'User: "이 프로젝트를 리팩토링해줘 단계마다 확인받고 진행해" → {"mode":"task","tools":["filesystem","shell"]}',
  'User: "배포 파이프라인 만들어줘 각 단계에서 승인 필요" → {"mode":"task","tools":["filesystem","shell"]}',
  'User: "데이터 마이그레이션 진행해줘 각 테이블마다 내 승인 받고" → {"mode":"task","tools":["filesystem","shell"]}',
  "",
  "# Tool Category Selection",
  "In addition to the mode, return which tool categories are needed for this request.",
  "Available categories: filesystem, shell, web, messaging, file_transfer, scheduling, memory, decision, promise, secret, diagram, admin, spawn",
  "",
  "Rules:",
  "- Return only the categories the request actually needs as a JSON array in the 'tools' field",
  "- messaging and file_transfer are always available — include only if the request explicitly needs them",
  "- For builtin/inquiry modes, omit the tools field",
  "- If unsure which tools are needed, omit the tools field entirely (system will use mode defaults)",
  "",
  "## tool selection examples",
  'User: "날씨 알려줘" → {"mode":"once","tools":["web"]}',
  'User: "파일 읽고 수정해줘" → {"mode":"agent","tools":["filesystem"]}',
  'User: "웹 검색해서 파일로 저장해줘" → {"mode":"agent","tools":["web","filesystem"]}',
  'User: "크론 등록해줘" → {"mode":"once","tools":["scheduling"]}',
  'User: "코드 분석하고 리포트 만들어줘" → {"mode":"agent","tools":["filesystem","shell","web"]}',
  'User: "다이어그램 그려줘" → {"mode":"once","tools":["diagram"]}',
  'User: "API 호출해서 데이터 가져와" → {"mode":"once","tools":["web"]}',
  'User: "메모리에 저장해줘" → {"mode":"once","tools":["memory"]}',
  "",
  "# Node Category Selection (for phase/workflow mode)",
  "When mode is \"phase\", also return which workflow node categories are needed in a 'nodes' array.",
  "Available node categories: flow, data, ai, integration, advanced",
  "- flow: branching (if/switch), loops, merging, filtering, waiting — always included automatically",
  "- data: variable assignment, templates, code execution, DB queries, file I/O, aggregation",
  "- ai: LLM calls, AI agents, analyzers, embeddings, vector search, text splitting",
  "- integration: HTTP requests, OAuth, webhooks, notifications, file sending",
  "- advanced: task execution, decisions, promises",
  "",
  "## node selection examples",
  'User: "API에서 데이터 가져와서 분석해줘" → {"mode":"phase","nodes":["data","ai","integration"]}',
  'User: "사용자 입력을 분류하고 결과를 알려줘" → {"mode":"phase","nodes":["ai","integration"]}',
  'User: "매일 보고서 생성 워크플로우" → {"mode":"phase","nodes":["data","ai","integration"]}',
  'User: "코드로 데이터 처리하고 DB에 저장" → {"mode":"phase","nodes":["data"]}',
  'User: "웹훅 받아서 LLM으로 분석 후 슬랙 알림" → {"mode":"phase","nodes":["ai","integration"]}',
].join("\n");
...
export const BASE_FLOWCHART = [
  "",
  "# Decision Flowchart",
  "1. Is the user asking about the assistant's identity/name/role? Yes → identity",
...
export const INQUIRY_FLOWCHART = [
  ...
  "# Decision Flowchart",
  "1. Is the user asking about the assistant's identity/name/role? Yes → identity",
  "2. Are there active tasks AND does the user ask about their status/progress? Yes → inquiry",
  "3. Does it map to a built-in command? Yes → builtin",
  "4. Does the user request any action? No → once",
  "5. Explicit multi-agent parallel workflow? Yes → phase",
  "6. Does it combine 2+ distinct actions? No → once",
  "7. Approval gates, pause/resume, or multi-step with approval-requiring tools? Yes → task",
  "8. Otherwise → agent",
].join("\n");

// ── 가드 요약 프롬프트 ──

export const GUARD_SUMMARY_PROMPT = [
  "You are a task summarizer. The user submitted a request to an AI assistant.",
  "Summarize what the assistant will do in 1-2 sentences in Korean.",
  "Focus on: what action, what resources, what impact.",
