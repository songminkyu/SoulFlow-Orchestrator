...
export const CONFIG_FIELDS: ConfigFieldMeta[] = [
  ...
  { path: "channel.pollMaxIntervalMs", label: "Poll Max Interval (ms)", section: "channel", type: "number", env_key: "CHANNEL_POLL_MAX_INTERVAL_MS", default_value: 0, sensitive: false, restart_required: false, description: "Idle poll backoff ceiling (0 = disabled). When set, poll interval grows 1.5× per empty cycle up to this limit, then resets on activity." },
  ...
  { path: "channel.queueCapPerLane", label: "Queue Cap Per Lane", section: "channel", type: "number", env_key: "CHANNEL_QUEUE_CAP_PER_LANE", default_value: 20, sensitive: false, restart_required: false, description: "Max pending messages per session lane. 0 = unlimited. Excess messages are dropped per queueDropPolicy." },
  { path: "channel.queueDropPolicy", label: "Queue Drop Policy", section: "channel", type: "select", env_key: "CHANNEL_QUEUE_DROP_POLICY", default_value: "old", sensitive: false, restart_required: false, options: ["old", "new"], description: "When queue cap is exceeded: old = remove oldest pending, new = reject incoming" },
  { path: "channel.inboundDebounce.enabled", label: "Inbound Debounce", section: "channel", type: "boolean", env_key: "CHANNEL_INBOUND_DEBOUNCE_ENABLED", default_value: false, sensitive: false, restart_required: false, description: "Batch rapid consecutive messages from the same chat into a single invocation" },
  { path: "channel.inboundDebounce.windowMs", label: "Debounce Window (ms)", section: "channel", type: "number", env_key: "CHANNEL_INBOUND_DEBOUNCE_WINDOW_MS", default_value: 400, sensitive: false, restart_required: false, description: "Time to wait after first message before processing the batch" },
  { path: "channel.inboundDebounce.maxMessages", label: "Debounce Max Messages", section: "channel", type: "number", env_key: "CHANNEL_INBOUND_DEBOUNCE_MAX_MESSAGES", default_value: 5, sensitive: false, restart_required: false, description: "Flush immediately when this many messages are buffered" },
  { path: "channel.sessionLanePruneIntervalMs", label: "Lane Prune Interval (ms)", section: "channel", type: "number", env_key: "CHANNEL_SESSION_LANE_PRUNE_INTERVAL_MS", default_value: 300_000, sensitive: false, restart_required: false, description: "How often to clean up idle session lanes (0 = disabled)" },
  { path: "channel.staleRunTimeoutMs", label: "Stale Run Timeout (ms)", section: "channel", type: "number", env_key: "CHANNEL_STALE_RUN_TIMEOUT_MS", default_value: 7_200_000, sensitive: false, restart_required: false, description: "Abort active runs that have been running longer than this TTL (0 = disabled)" },
  ...
  { path: "channel.streaming.coalesceMaxChars", label: "Coalesce Max Chars", section: "channel.streaming", type: "number", env_key: "CHANNEL_STREAMING_COALESCE_MAX_CHARS", default_value: 300, sensitive: false, restart_required: false, description: "Force-flush the stream buffer when accumulated content exceeds this length, regardless of intervalMs. 0 = disabled." },
  ...
  // ── Memory & Consolidation ──
  { path: "memory.consolidation.enabled", label: "Consolidation Enabled", section: "memory.consolidation", type: "boolean", env_key: "", default_value: true, sensitive: false, restart_required: false, description: "Automatically compress conversation history into long-term memory" },
  ...
  { path: "memory.consolidation.idleAfterMs", label: "Idle After (ms)", section: "memory.consolidation", type: "number", env_key: "", default_value: 300_000, sensitive: false, restart_required: false, description: "Wait time after last activity before triggering idle consolidation" },
  ...
  { path: "memory.consolidation.archiveUsed", label: "Archive After Consolidation", section: "memory.consolidation", type: "boolean", env_key: "", default_value: true, sensitive: false, restart_required: false, description: "Delete daily memory entries after they have been consolidated" },
  ...
  { path: "memory.longtermInjectionMaxChars", label: "Longterm Injection Max Chars", section: "memory.consolidation", type: "number", env_key: "", default_value: 20_000, sensitive: false, restart_required: false, description: "Character limit for longterm memory injected into system prompt (0 = unlimited, tail-first)" },
  ...
  { path: "orchestration.executorMaxTokens", label: "Executor Max Tokens", section: "orchestration", type: "number", env_key: "EXECUTOR_MAX_TOKENS", default_value: 4096, sensitive: false, restart_required: false, description: "Maximum output tokens for executor LLM calls. Increase for local vLLM/Ollama models (e.g. 8192-16384)" },
  { path: "orchestration.systemPromptMaxTokens", label: "System Prompt Budget", section: "orchestration", type: "number", env_key: "SYSTEM_PROMPT_MAX_TOKENS", default_value: 0, sensitive: false, restart_required: false, description: "Max tokens for system prompt. 0 = unlimited. Set for small-context OSS models (e.g. 4000-6000) to enable priority-based section pruning" },
