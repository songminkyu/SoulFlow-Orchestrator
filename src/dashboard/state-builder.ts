...
export async function build_merged_tasks(options: DashboardOptions): Promise<Array<{
  ...
  const loop_tasks = options.agent.list_runtime_tasks();
  const stored_tasks = await options.agent.list_stored_tasks();
...
export async function build_dashboard_state(
  ...
  const subagents = options.agent.list_subagents();
  const messages = recent_messages.slice(-20).reverse();
  ...
  const processes = {
    active: (tracker?.list_active() ?? []).map(map_process),
    recent: (tracker?.list_recent(20) ?? []).map(map_process),
  };

  const cron_sched = options.cron;
  const cron = cron_sched ? await (async () => {
    const [st, jobs] = await Promise.all([cron_sched.status(), cron_sched.list_jobs(true)]);
    return {
      ...st,
      jobs: jobs.map((j) => ({
  ...
  const approvals = options.agent.list_approval_requests("pending").map((a) => ({
    request_id: a.request_id, tool_name: a.tool_name, status: a.status,
    created_at: a.created_at, context: a.context,
  }));
  const active_loops = options.agent.list_active_loops().map((l) => ({
